...
public class Gun extends DynamicEntity{
    ...
    public boolean shoot(boolean hit, Wall w){
        if(current_animation_frame == 1 && bullets > 0){
            if(hit){
                sd.play(new File("C:/Users/Zhazted Rhixin/IdeaProjects/JMAZE/src/Audio/hit.wav"), 5f);
                w.state = true;
            }
            setIdleState(false);
            bullets -=1;
            sd.play(new File("C:/Users/Zhazted Rhixin/IdeaProjects/JMAZE/src/Audio/shoot_reload.wav"), 5f);
            return true;
        }

        if(bullets < 1){
            sd.play(new File("C:/Users/Zhazted Rhixin/IdeaProjects/JMAZE/src/Audio/no_ammo.wav"), 5f);
        }
